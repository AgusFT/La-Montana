...
**Ya se puede crear al propietario, registrar particulares, iniciar/cerrar sesiones y gestionar sucursales y empleados desde administración.** El empleado ingresa a su espacio y consulta únicamente sus sucursales habilitadas. Las tarifas y los servicios globales ya se pueden configurar y programar. La configuración operativa admite activación inmediata, programación, cancelación y ejecución automática recuperable. Oferta real de entregas, operación de recursos, historial completo, pedidos, movimientos de pagos, PDF y producción/entrega siguen **En construcción**. No hay cuentas ni datos comerciales precargados. V1 prepara el esquema; V2 agrega identidad, el rol técnico de administrador, el registro único de inicialización, eventos de acceso y tablas técnicas de sesiones. V3 incorpora el rol técnico CLIENTE para particulares, sin crear cuentas.
...
Después de revisar las advertencias y corregir los bloqueos en fase6, **Continuar a fase7** abre la decisión de aplicación. La selección comienza vacía. Elegir Activar ahora, ingresar un motivo y continuar a Verificación de seguridad; allí se reingresa la contraseña y se solicita el código del buzón Mailpit. La acción final vuelve a validar la revisión y activa la configuración. La entrega20 agrega programación operativa y la21 incorpora adelanto manual e intentos durables. El historial completo continúa pendiente.
...
La fase1 presenta los últimos intentos; una ejecución en curso bloquea edición y permite consultar nuevamente. La pantalla final distingue éxito de fallo/interrupción, informa cómo continuar y limpia credenciales. Ante una respuesta incierta conserva sólo en memoria el comando exacto para recuperarlo. Las condiciones previas y los intentos no sustituyen la futura pantalla de historial completo.
...
No hay semillas comerciales ni cuentas de muestra en el producto. Historial/detalle/comparación/base/rollback, cotizaciones, PDF, pagos, producción y entregas reales siguen dentro del alcance pendiente; la activación verificada no equivale a una demo E2E terminada.
