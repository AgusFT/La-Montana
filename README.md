...
El panel `/` explica el orden de instalación y permanece accesible con sesión. `/web` es la página pública local: antes de publicar muestra una estructura sin contenido comercial. Antes del propietario, login y registro explican el requisito; después, los clientes pueden registrarse sin activar pedidos. El dashboard muestra el mapa de preparación sólo durante la primera instalación, hasta activar correctamente una configuración. La instalación inicial avanza en orden obligatorio: sucursal → papeles → servicio base de impresión → tarifas y servicios publicados → modelo → pagos → impresoras y asignación → horarios y entrega → revisión y activación. El menú, las URL de las pantallas y las fases internas impiden adelantar pasos incompletos, usando requisitos guardados y validados por el servidor. Los pasos anteriores siguen disponibles para corregirlos. Web, empleados y simulaciones son opcionales; las simulaciones se presentan plegadas. Las reglas de las API de escritura se conservan: este recorrido es una restricción de navegación e interacción del frontend. Un borrador o una primera activación programada todavía mantienen el recorrido visible. Después de activar, el mapa desaparece aunque queden complementos pendientes o se preparen nuevas configuraciones; el configurador sigue disponible desde el menú. El estado se consulta al ingresar, volver a la pestaña o restaurar la página, sin mostrar el mapa antes de conocerlo. El configurador web permite importar imágenes de una carpeta local, guardar identidad/fichas/secciones, revisar en privado y publicar expresamente. Los formularios remotos permanecen deshabilitados con «implementacion en desarrollo».
...
### Primera instalación guiada y confirmaciones

Cada fase y subfase informa qué falta antes de habilitar la siguiente. El catálogo exige una sucursal activa, papeles habilitados y un servicio base de impresión. En precios se confirma primero una tarifa, después un servicio de impresión habilitado y por último se guarda la configuración completa. Una configuración comercial programada no habilita la operación hasta estar vigente.
