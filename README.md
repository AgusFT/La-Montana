...
Siguiente entrega: cobertura domiciliaria con costos/franjas/cupos, completar mínimos de fase5 y fases6–8 (simulación integral, activación, programación e historial). Continuar luego PDF/pedidos/pagos y operación/entrega. La disponibilidad temporal actual usa las identidades presentes en el borrador antes de la primera activación: al incorporar versiones vigentes se debe consultar su conjunto de puntos y revalidar la disponibilidad al ofrecer/confirmar, sin mezclar estados cotidianos con snapshots. Docker sigue pendiente de certificación; no repetir el intento sin cambio de entorno.
...
Guardar esta selección **no activa reglas ni habilita pedidos**. La entrega 11 implementa parámetros financieros y ejemplos calculados de fase 3. La entrega12 agrega recursos/capacidades declarados y servicios por sucursal. La entrega13 agrega calendarios y ejemplos temporales; oferta real de entregas, simulación integral, activación segura e historial operativo siguen En construcción. La entrega 10 habilita cancelar el borrador con contraseña y código por correo, como exige el mock; también se puede seguir editando el existente.
...
Zonas domiciliarias con sus costos/franjas/cupos, reservas y oferta real de entrega **siguen dentro de v0.1 y pendientes**, marcados En construcción. Habilitar un checkbox no certifica una modalidad: se exigen destinos utilizables cuando sólo se ofrecen puntos; con retiro local válido, la falta de puntos utilizables genera un aviso. El envío domiciliario sigue sin superar la validación hasta incorporar su cobertura. La validación local de retiro no sustituye la futura revisión integral, la reserva atómica ni la activación segura.
...
Antes de la primera activación se presentan sólo las identidades del borrador, con aviso **Sin configuración activa**. El estado temporal persiste por identidad si se cancela ese borrador, pero las definiciones históricas no se presentan como puntos ofrecibles. Al implementar activación debe consultarse el conjunto vigente y revalidarse su disponibilidad al ofrecer y confirmar nuevos pedidos. La oferta, las reservas y la gestión de compromisos existentes siguen En construcción: esta entrega no crea, reasigna ni cancela pedidos. La operación por empleados se integrará con los permisos y bandejas correspondientes.

V15 agrega `disponibilidad_punto_entrega` y `evento_disponibilidad_punto`, separadas de las tablas versionadas. El evento conserva operación, actor, estado anterior/nuevo, versión operativa y fecha. La versión de disponibilidad sirve para rechazar cambios concurrentes; no es una versión de configuración. No se modifica ni se revoca una autorización ligada exclusivamente al contenido del borrador por cambiar disponibilidad. La futura activación debe revalidar elegibilidad actual, además de comprobar el contenido autorizado.
