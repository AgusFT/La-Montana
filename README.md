...
## Estado actual · entrega 36

El flujo principal está implementado, incluida la reprogramación aceptada por el cliente. La última verificación aprobó 154 pruebas de backend, build/tipado de frontend y tres recorridos de navegador hasta cierre, con dos sucursales y empleados aislados. Reiniciar los procesos locales conservó configuración, pedidos, dinero, PDF y sesiones. **La demo completa sigue en verificación:** faltan la regresión integrada final de D2/versiones, la revisión visual restante y certificar el arranque y los volúmenes de Docker Compose. El entorno de pruebas local no sustituye esa certificación.
...
El PDF usa un visor privado compartido; las etapas del pedido se navegan por secciones. El código secreto sólo aparece al cliente, adaptando el ejemplo que lo mostraba al administrador. No hay mock específico del diálogo de reprogramación en main; sigue el contrato del DER §7.6. La comparación visual integral, incluida la coherencia del dashboard/marcos administrativos, sigue pendiente; no se afirma fidelidad final de toda la aplicación.
