...
El panel `/` explica el orden de instalación y permanece accesible con sesión iniciada. `/web` es el espacio público local: antes de publicar muestra una estructura sin contenido comercial. Antes del propietario, login y registro explican el requisito; después, las cuentas de clientes se pueden crear sin activar pedidos. El dashboard muestra un recorrido calculado con datos y validadores reales: distingue borrador/versión activa, requisitos pendientes y empleados opcionales, y ofrece reintento si falla la consulta. El Configurador de página web permite guardar identidad, contacto, fichas y secciones, revisar una vista previa privada y publicar expresamente. Las fichas ocultas y los datos de contacto de una sección desmarcada no se exponen en la respuesta pública. La biblioteca ya importa JPEG, PNG y WebP desde una carpeta local autorizada y permite asociarlos a la identidad y a las fichas. Las copias y miniaturas son persistentes; sólo las imágenes de la publicación vigente son públicas. La integración y certificación final de v6 continúan en las entregas siguientes. Los formularios remotos están deshabilitados con «implementacion en desarrollo».
