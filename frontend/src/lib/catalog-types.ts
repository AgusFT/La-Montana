...
export type RevisionSummary = { codigoPublico: string; numero: number; motivo: string; creadaEn: string; actor: string };
export type CatalogRevision = RevisionSummary & { tarifas: PrintRate[]; servicios: ServiceOffer[] };
export type CatalogState = { formatos: Format[]; papeles: Paper[]; servicios: CatalogService[]; actual: CatalogRevision | null; historial: RevisionSummary[] };
export type NewRevision = { versionBase: string | null; operacion: string; motivo: string; tarifas: PrintRate[]; servicios: ServiceOffer[] };
...
function summary(value: unknown): value is RevisionSummary { return record(value) && strings(value,["codigoPublico","motivo","creadaEn","actor"]) && number(value.numero); }
...
export function isCatalogState(value: unknown): value is CatalogState {
  return record(value) && Array.isArray(value.formatos) && value.formatos.every(f=>record(f)&&strings(f,["codigoPublico","codigo","nombre"])&&number(f.anchoMm)&&number(f.altoMm)) &&
    ...
    (value.actual===null||isCatalogRevision(value.actual)) && Array.isArray(value.historial) && value.historial.every(summary);
}
