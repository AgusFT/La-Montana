...
export type PointAvailabilityPanel={contexto:"SIN_CONFIGURACION_ACTIVA";borrador:string|null;puntos:OperationalPoint[];deshabilitados:number};
...
export function isPointAvailabilityPanel(v:unknown):v is PointAvailabilityPanel{return record(v)&&v.contexto==="SIN_CONFIGURACION_ACTIVA"&&(v.borrador===null||typeof v.borrador==="string"&&uuidPattern.test(v.borrador))&&Number.isSafeInteger(v.deshabilitados)&&Number(v.deshabilitados)>=0&&Array.isArray(v.puntos)&&v.puntos.every(p=>record(p)&&["codigoPublico","codigo","nombre","direccion","zonaHoraria"].every(k=>typeof p[k]==="string")&&isPointAvailability(p.disponibilidad)&&p.disponibilidad.punto===p.codigoPublico);}
