...
export type ConfigurationAttempt={codigo:string;configuracion:string;numero:number;origen:string;estado:string;iniciadoEn:string;terminadoEn:string|null;atrasoDetectadoEn:string|null;resultado:string|null;detalle:string|null};
...
export function isConfigurationAttempt(i:unknown):i is ConfigurationAttempt{return record(i)&&["codigo","configuracion","origen","estado","iniciadoEn"].every(k=>typeof i[k]==="string")&&Number.isSafeInteger(i.numero)&&["terminadoEn","atrasoDetectadoEn","resultado","detalle"].every(k=>i[k]===null||typeof i[k]==="string");}
