...
export function PrivateFiles({quote,version,internal=false,refresh=0,onChange,confirmed=false,snapshot,correction}:{quote:string;version:number;internal?:boolean;refresh?:number;onChange?:()=>void;confirmed?:boolean;snapshot?:string[];correction?:{pedido:string;solicitud:string}}){
 ...
 async function load(){try{const data=await quoteRead(base,isFileView);setView(snapshot?{...data,items:data.items.map(i=>({...i,archivos:i.archivos.filter(f=>snapshot.includes(f.codigoPublico))}))}:data);setPreview(current=>current?data.items.flatMap(i=>i.archivos).find(f=>f.codigoPublico===current.codigoPublico)??current:null);}catch(e){setError(e instanceof Error?e.message:"No pudimos consultar los archivos.");}}
 useEffect(()=>{void load();},[quote,version,internal,refresh,snapshot?.join(":")]);
 ...
 function upload(item:string){if(locked||!view?.carga.habilitada||!view.items.find(i=>i.codigoPublico===item)?.admiteCarga)return;const file=selected[item];if(!file)return;if(file.size===0||file.size>10*1024*1024||!file.name.toLowerCase().endsWith(".pdf")){setError("Seleccioná un PDF de hasta 10 MiB.");return;}
  ...
  command.current={kind:"upload",item,file,body:JSON.stringify({operacion:crypto.randomUUID(),item,version:view.version,correccion:view.correccion}),id:pending?.codigoPublico};void execute();
 }
 ...
 return <section className="client-card private-files"><h2>PDF privados y vista previa</h2><p>{confirmed&&snapshot?"Estos son los PDF inspeccionados y aceptados vinculados al pedido. Podés consultar las páginas o descargar los originales.":"El sistema analiza el contenido y genera las páginas de vista previa. Aceptarlas no confirma todavía el pedido ni registra un pago."}</p>
 {view?<><p className={view.carga.habilitada?"client-note":"client-warning"}>{confirmed?"Consulta de versiones guardadas. Las correcciones se responden explícitamente desde el pedido.":view.carga.motivo}</p>{view.items.map((item,index)=><section className="private-file-item" key={item.codigoPublico}><h3>Archivo {index+1}: {item.nombre}</h3>
 {!internal&&!confirmed&&item.admiteCarga&&view.carga.habilitada&&<div className="client-form"><label>Seleccionar PDF para el archivo {index+1}<input key={item.archivos.map(f=>f.codigoPublico+f.estado).join(":")} type="file" accept="application/pdf,.pdf" disabled={locked} onChange={e=>{const f=e.target.files?.[0];if(f)setSelected(old=>({...old,[item.codigoPublico]:f}));}}/></label><button type="button" className="client-button" disabled={locked||!selected[item.codigoPublico]||item.archivos.some(f=>f.estado==="VALIDANDO")} onClick={()=>upload(item.codigoPublico)}>{busy?"Procesando…":"Enviar PDF para analizar"}</button></div>}
 {item.archivos.length===0?<p className="client-muted">Todavía no hay un PDF recibido para este ítem.</p>:<ul className="private-file-history">{item.archivos.map(f=><li key={f.codigoPublico}><strong>{stateNames[f.estado]??f.estado}{snapshot?" · versión del pedido":f.activo?" · última carga":""}</strong><span>{date(f.creadoEn)}{f.paginas?` · ${f.paginas} páginas`:""}</span><p>{f.aceptadaEn?"PDF inspeccionado; esta versión tiene su vista previa aceptada.":f.mensaje??(f.estado==="VALIDANDO"?"Analizando el archivo; podés recuperar el estado al recargar.":"Intento preparado. Seleccioná el PDF y envialo antes del vencimiento.")}</p>{f.aceptadaEn&&<p className="client-success">Vista previa aceptada el {date(f.aceptadaEn)}.</p>}{["VALIDO","REQUIERE_COTIZACION"].includes(f.estado)&&<div className="client-actions"><button className="client-button secondary" disabled={locked} onClick={()=>open(f)}>Ver páginas{snapshot?" del pedido":f.activo?" de esta carga":""}</button><a href={locked?undefined:`${base}/${f.codigoPublico}/original`} className="client-button secondary">Descargar PDF inspeccionado</a>{f.estado==="REQUIERE_COTIZACION"&&!internal&&<a className="client-button" href={locked?undefined:`/cliente/cotizaciones/nueva?reemplaza=${quote}&archivo=${f.codigoPublico}${correction?`&pedido=${correction.pedido}&solicitud=${correction.solicitud}`:""}`}>Recotizar con datos inspeccionados</a>}</div>}</li>)}</ul>}
 </section>)}</>:!error&&<p role="status">Consultando archivos…</p>}
 {ready&&preview&&<section className="private-preview" aria-label="Vista previa del PDF"><h3>Vista previa · {preview.nombre}</h3><p>{pages} páginas inspeccionadas · {snapshot?"Versión del pedido":preview.activo?"Última carga":"Versión histórica o pendiente de recotizar"}</p><div className="client-actions"><button className="client-button secondary" disabled={locked||page<=1} onClick={()=>{setPage(p=>p-1);setImageError(false);}}>Página anterior</button><label>Página<input type="number" min={1} max={pages} value={page} disabled={locked} onChange={e=>{const n=Number(e.target.value);if(Number.isInteger(n)&&n>=1&&n<=pages){setPage(n);setImageError(false);}}}/></label><span>de {pages}</span><button className="client-button secondary" disabled={locked||page>=pages} onClick={()=>{setPage(p=>p+1);setImageError(false);}}>Página siguiente</button></div>
