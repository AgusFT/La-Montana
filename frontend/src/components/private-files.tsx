...
export function PrivateFiles({quote,version,internal=false,refresh=0,onChange,confirmed=false}:{quote:string;version:number;internal?:boolean;refresh?:number;onChange?:()=>void;confirmed?:boolean}){
 ...
 return <section className="client-card private-files"><h2>PDF privados y vista previa</h2><p>{confirmed?"Estos son los PDF inspeccionados y aceptados vinculados al pedido. Podés consultar las páginas o descargar los originales.":"El sistema analiza el contenido y genera las páginas de vista previa. Aceptarlas no confirma todavía el pedido ni registra un pago."}</p>
 {view?<><p className={view.carga.habilitada?"client-note":"client-warning"}>{confirmed&&internal?"Consulta interna de los PDF confirmados. Registrá la revisión y las decisiones desde el pedido.":view.carga.motivo}</p>{view.items.map((item,index)=><section className="private-file-item" key={item.codigoPublico}><h3>Archivo {index+1}: {item.nombre}</h3>
