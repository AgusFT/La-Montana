...
export function DraftCancellation({draft,onCancelled,onRefresh,onLockChange}:{draft:ConfigurationDraft;onCancelled:(value:ConfigurationDraft)=>Promise<void>;onRefresh:()=>Promise<void>;onLockChange:(locked:boolean)=>void}){
  ...
  async function send(kind?:"solicitar"|"confirmar",event?:FormEvent<HTMLFormElement>){
    ...
    try{
      if(kind){if(!operation.current)operation.current=crypto.randomUUID();const data={operacion:operation.current,version:draft.version,motivo:reason.trim(),contrasena:password,...(kind==="confirmar"?{codigo:code.trim()}:{})};pending.current={kind,body:JSON.stringify(data)};}
      if(!pending.current)throw new Error("No hay una solicitud pendiente.");const command=pending.current;
      const response=await secureMutation(`/api/admin/configuracion/borradores/${draft.codigoPublico}/cancelacion/${command.kind}`,command.body,"application/json");const data:unknown=await response.json();
  ...
  if(!open)return <section className="admin-card"><h3>Administrar el cambio pendiente</h3><p className="admin-note">Cancelar conserva el borrador en el historial y permite comenzar otro. Se confirma con tu contraseña y un código enviado a tu correo.</p><button type="button" className="admin-button secondary" onClick={()=>setOpen(true)}>Cancelar borrador</button></section>;
  return <section className="admin-card draft-cancellation" aria-labelledby="cancel-draft-title">
    <h3 id="cancel-draft-title">Cancelar borrador {draft.numero}</h3><p className="admin-note">Edición {draft.version}. Se conservarán su selección, responsable, fecha y motivo de cancelación. Los precios del catálogo no se modifican.</p>
