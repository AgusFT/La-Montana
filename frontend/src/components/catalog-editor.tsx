...
export function CatalogEditor({catalog,onSaved}:{catalog:CatalogState;onSaved:(revision:CatalogRevision)=>void}){
  ...
  return <section id="revision-comercial" className="pricing-editor"><form onSubmit={save} className="admin-form">
    ...
    <div className="pricing-guide"><strong>Configurá en este orden</strong><ol><li><b>1. Tarifas</b><span>Cuánto vale imprimir en cada papel y color.</span></li><li><b>2. Servicios</b><span>Qué trabajos ofrecés y cómo cobrás sus terminaciones.</span></li><li><b>3. Aplicación</b><span>Revisá el conjunto y elegí cuándo usar estos precios.</span></li></ol><p>Los precios finales se muestran en ARS. Para doble faz podés ingresar el precio final o calcularlo sumando un importe o un porcentaje sobre simple faz. Escribí, por ejemplo, <b>1500,50</b> o <b>1500.50</b>, sin separadores de miles.</p></div>
    <fieldset disabled={locked} className="admin-fieldset">
      <section className="admin-card pricing-section" aria-labelledby="pricing-rates-title">
        ...
        <div className="pricing-card-list">{rates.map((rate,index)=><CatalogRateEditor key={rate.id} rate={rate} index={index} rates={rates} catalog={catalog} onChange={change=>changeRate(rate.id,change)} onRemove={()=>{setSaved(null);setRates(current=>current.filter(row=>row.id!==rate.id));}}/>)}</div>
      </section>
