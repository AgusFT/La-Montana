 ...
 * ------------------------------------------------------------------------
 * TIPOS DECLARADOS
 * - PairDraft (TypeAliasDeclaration).
 * - OfferDraft (TypeAliasDeclaration).
 * ========================================================================
 */
//#endregion
...
export function CatalogEditor({catalog,onSaved}:{catalog:CatalogState;onSaved:(revision:CatalogRevision)=>void}){
  ...
  const baselineRates=seedRates(baseline);
  const dirty=!!editingRate||JSON.stringify(rates.map(rateSignature))!==JSON.stringify(baselineRates.map(rateSignature))||JSON.stringify(offers)!==JSON.stringify(seedOffers(baseline));
  ...
  function payload():NewRevision{
    ...
    return {versionBase:base,operacion:crypto.randomUUID(),motivo:reason.trim(),programadaPara:date,tarifas:expanded,servicios:offers.map(({servicio,nombreVisible,basePrecio,precio,preparacionMinutos,habilitado,compatibilidades})=>({servicio,nombreVisible:nombreVisible.trim(),basePrecio:basePrecio as PriceBase,precio:parseAmount(precio,"Precio del servicio"),preparacionMinutos:Number(preparacionMinutos),habilitado,compatibilidades:compatibilidades.map(({formato,papel})=>({formato,papel}))}))};
  }
  async function save(event?:FormEvent<HTMLFormElement>){
    ...
    try{
      ...
      pending.current=null;setUncertain(false);setConflict(data.estado!=="VIGENTE"&&data.estado!=="PROGRAMADA");setComparison(null);if(data.estado==="VIGENTE"){setBase(data.codigoPublico);setBaseNumber(data.numero);}setReason("");setRates(seedRates(data));setOffers(seedOffers(data));setBaseline(data);setEditingRate(null);setRateError("");setSaved(data);onSaved(data);
    }catch(error){
  ...
  return <section id="revision-comercial" className="pricing-editor"><form onSubmit={save} className="admin-form">
    <div className="catalog-editor-heading"><div><h2>Tarifas y servicios</h2><p>{baseNumber===null?"Todavía no hay precios vigentes. Prepará las tarifas y los servicios y guardá la primera configuración.":dirty?`Tenés cambios pendientes sobre la configuración ${baseNumber}. Los precios vigentes se conservan hasta guardar.`:baseline?.estado==="PROGRAMADA"?"Estos precios están guardados y programados; se aplicarán en la fecha elegida.":`Configuración vigente ${baseNumber}. Las tarifas ya están aplicadas; abrí Editar tarifa sólo si querés cambiarlas.`}</p></div><span className="pricing-currency">ARS · Pesos argentinos</span></div>
    <div className="pricing-guide"><strong>Configurá en este orden</strong><ol><li><b>1. Tarifas</b><span>Editá y aplicá cada tarifa al borrador.</span></li><li><b>2. Servicios</b><span>Qué trabajos ofrecés y cómo cobrás sus terminaciones.</span></li><li><b>3. Publicación</b><span>Guardá la configuración completa para aplicar sus precios.</span></li></ol><p>Los precios finales se muestran en ARS. Para doble faz podés ingresar el precio final o calcularlo sumando un importe o un porcentaje sobre simple faz. Escribí, por ejemplo, <b>1500,50</b> o <b>1500.50</b>, sin separadores de miles.</p></div>
