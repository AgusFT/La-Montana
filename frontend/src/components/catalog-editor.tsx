  ...
  return <section id="revision-comercial"><form onSubmit={save} className="admin-form">
    ...
    <fieldset disabled={locked} className="admin-fieldset">
      <div className="catalog-two-columns">
        <section className="admin-card"><div className="admin-section-title"><div><h2>Tarifas de impresión</h2><p>Elegí un papel con su tamaño incluido y definí el precio por carilla y color.</p></div><button type="button" className="admin-button" disabled={rates.length>=300||!catalog.papelesHabilitados.some(p=>p.habilitado)} onClick={()=>setRates(current=>[...current,{id:crypto.randomUUID(),formato:"",papel:"",color:"",precio:"",recargoDobleFaz:"",habilitada:false}])}>+ Agregar tarifa</button></div>
          {rates.length===0?<p className="admin-empty">No hay tarifas en esta revisión. Habilitá papeles en el catálogo base para comenzar.</p>:<div className="admin-table-wrap" tabIndex={0} aria-label="Edición de tarifas"><table className="admin-table catalog-edit-table"><thead><tr><th>Papel con tamaño</th><th>Color</th><th>Precio / recargo doble faz</th><th>Estado</th><th>Acción</th></tr></thead><tbody>{rates.map((rate,index)=><tr key={rate.id}>
            <td><CatalogPaperSelect catalog={catalog} value={rate} label={`Papel de tarifa ${index+1}`} onChange={pair=>changeRate(rate.id,pair)}/></td>
        ...
        <section className="admin-card"><div className="admin-section-title"><div><h2>Servicios ofrecidos</h2><p>Impresión y terminaciones, con sus condiciones.</p></div><button type="button" className="admin-button" disabled={offers.length>=100||!catalog.servicios.length} onClick={()=>setOffers(current=>[...current,{id:crypto.randomUUID(),servicio:"",nombreVisible:"",basePrecio:"",precio:"",preparacionMinutos:"",habilitado:false,compatibilidades:[]}])}>+ Agregar servicio</button></div>
          ...
          </tr>{!printing&&<tr><td colSpan={4}><fieldset className="catalog-compatibility"><legend>Compatibilidades de {offer.nombreVisible||`servicio ${index+1}`}</legend>{offer.compatibilidades.length===0&&<p className="admin-note">Elegí al menos un papel con tamaño si la terminación estará habilitada.</p>}{offer.compatibilidades.map((pair,pairIndex)=><div className="catalog-pair" key={pair.id}><CatalogPaperSelect catalog={catalog} value={pair} label={`Papel compatible ${pairIndex+1} de servicio ${index+1}`} onChange={p=>pairChange(offer,pair.id,p)}/><button type="button" className="admin-link-button" onClick={()=>changeOffer(offer.id,{compatibilidades:offer.compatibilidades.filter(p=>p.id!==pair.id)})}>Quitar</button></div>)}<button type="button" className="admin-link-button" disabled={offer.compatibilidades.length>=300||!catalog.papelesHabilitados.some(p=>p.habilitado)} onClick={()=>changeOffer(offer.id,{compatibilidades:[...offer.compatibilidades,{id:crypto.randomUUID(),formato:"",papel:""}]})}>+ Agregar compatibilidad</button></fieldset></td></tr>}</Fragment>;})}</tbody></table></div>}
