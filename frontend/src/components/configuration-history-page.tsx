...
export function ConfigurationHistoryPage(){const[locked,setLocked]=useState(false);useNavigationGuard({blocked:locked});return <div className="configuration-layout"><aside className="configuration-steps"><h2>Flujo de configuración</h2><p>Fase 8 de 8</p><nav aria-label="Fases del configurador">{phases.map((label,i)=><button key={i} aria-current={i===7?"step":undefined} disabled={locked||i>0&&i<7} onClick={()=>{if(i===0)location.assign("/administracion/configuracion");}}><span>{String(i+1).padStart(2,"0")}</span><span>{label}</span></button>)}</nav></aside><div className="configuration-body"><ConfigurationHistory onLockChange={setLocked} onCurrent={()=>location.assign("/administracion/configuracion")}/></div></div>;}
