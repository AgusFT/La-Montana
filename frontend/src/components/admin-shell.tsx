 ...
 * del usuario, selector claro/oscuro y cierre de sesión, según administración u operación.
 *
 * ------------------------------------------------------------------------
...
export function AdminShell({ title, description, name, children, active="catalogo", userRole="Propietario", administration=true }: { title:string; description:string; name:string; userRole?:string; administration?:boolean; children:React.ReactNode; active?:"web"|"dashboard"|"catalogo"|"configuracion"|"puntos"|"operacion"|"sucursales"|"empleados" }) {
  return <NavigationBoundary><div className={`admin-layout${active==="configuracion"?" configuration-mode":""}`}>
    <aside className="admin-sidebar"><a href={administration?"/administracion":"/operacion"} className="admin-brand"><MountainMark sun/><span>La Montaña<small>IMPRESIONES</small></span></a><p className="admin-nav-label">{administration?"ADMINISTRACIÓN":"OPERACIÓN"}</p>
      <nav aria-label={administration?"Navegación administrativa":"Navegación operativa"}>{administration&&<><a href="/administracion" aria-current={active==="dashboard"?"page":undefined}><AdminIcon kind="home"/>Dashboard</a><a href="/administracion/catalogo" aria-current={active==="catalogo"?"page":undefined}><AdminIcon kind="catalog"/>Servicios y precios</a><a href="/administracion/configuracion" aria-current={active==="configuracion"?"page":undefined}><AdminIcon kind="lock"/>Configurador</a><a href="/administracion/pagina-web" aria-current={active==="web"?"page":undefined}><AdminIcon kind="home"/>Configurador de página web</a><a href="/administracion/puntos-entrega" aria-current={active==="puntos"?"page":undefined}><AdminIcon kind="branch"/>Puntos de entrega</a><a href="/administracion/sucursales" aria-current={active==="sucursales"?"page":undefined}><AdminIcon kind="branch"/>Sucursales</a><a href="/administracion/empleados" aria-current={active==="empleados"?"page":undefined}><AdminIcon kind="people"/>Empleados</a></>}<a href="/operacion" aria-current={active==="operacion"?"page":undefined}><AdminIcon kind="branch"/>Operación y cobros</a><a href="/cuenta/seguridad"><AdminIcon kind="lock"/>Seguridad de la cuenta</a></nav>
    ...
    <div className="admin-main"><header className="admin-topbar"><span>{administration?"Administración de la imprenta":"Operación de sucursal"}</span><div className="admin-user"><span className="admin-avatar" aria-hidden="true">{name.slice(0,1).toUpperCase()}</span><div><strong>{name}</strong><small>{userRole}</small></div><LogoutButton/></div></header>
      <main className="admin-content"><p className="admin-breadcrumb"><a href={administration?"/administracion":"/operacion"}>{administration?"Administración":"Operación"}</a><span aria-hidden="true">›</span>{title}</p><div className="admin-title"><AdminIcon kind="catalog"/><div><h1>{title}</h1><p>{description}</p></div></div>{children}</main>
    </div>
  </div></NavigationBoundary>;
}
