...
async function proxy(request: Request, context: { params: Promise<{ path: string[] }> }) {
  ...
  const methods = reviewMethods ?? availabilityMethods ?? pointMethods ?? deliveryMethods ?? resourceMethods ?? ( (path.length === 2 || path.length === 3) && Object.hasOwn(allowed, route) ? allowed[route]
