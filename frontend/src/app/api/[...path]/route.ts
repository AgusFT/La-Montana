...
async function proxy(request: Request, context: { params: Promise<{ path: string[] }> }) {
  ...
  const activationMethods = configDraft && path.length === 6 && path[4] === "activacion" && ["solicitar","confirmar","revocar"].includes(path[5]) ? ["POST"] : null;
  const reviewMethods = configDraft && path[4] === "revision" ? path.length === 5 ? ["GET"] : path.length === 6 && path[5] === "simular" ? ["POST"] : null : null;
  ...
  const methods = activationMethods ?? reviewMethods ?? availabilityMethods ?? pointMethods ?? deliveryMethods ?? resourceMethods ?? ( (path.length === 2 || path.length === 3) && Object.hasOwn(allowed, route) ? allowed[route]
