...
export default async function AdministrationPage() {
  ...
  return <IdentityShell title={`Hola, ${owner.nombre}`} description="Tu sesión administrativa está activa.">
    <div className="owner-profile"><h2>{owner.nombre} {owner.apellido}</h2><p>{owner.correo}</p><span className="environment">Administración de la imprenta</span></div>
    ...
    <dl className="capabilities"><div><dt>Gestión de clientes</dt><dd>En construcción</dd></div><div><dt>Configuración de la imprenta</dt><dd>Preparación, revisión, activación, programación e historial/comparación disponibles; reutilización con revisión obligatoria disponible; reversión manual autorizada disponible</dd></div><div><dt>Pedidos y operación</dt><dd>Revisión, correcciones, producción manual, calidad, entrega, reprogramación y cierre disponibles por sucursal</dd></div></dl>
    <div className="page-actions"><a className="refresh" href="/administracion/configuracion/historial">Historial de configuraciones</a><a className="refresh" href="/administracion/configuracion">Configurar la imprenta</a><a className="refresh" href="/administracion/puntos-entrega">Disponibilidad de puntos</a><a className="refresh" href="/administracion/catalogo">Servicios y precios</a><a className="refresh" href="/administracion/sucursales">Administrar sucursales</a><a className="refresh" href="/administracion/empleados">Administrar empleados</a><a className="secondary-link" href="/operacion">Ver sucursales operativas</a><a className="secondary-link" href="/cuenta/seguridad">Seguridad de la cuenta</a><LogoutButton /></div>
  </IdentityShell>;
}
