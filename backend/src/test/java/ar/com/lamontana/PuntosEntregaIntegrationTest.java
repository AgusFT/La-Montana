...
class PuntosEntregaIntegrationTest {
    ...
    @Test void estructuraVaciaCompartidaPorDosOrigenesCostosCuposYDisponibilidadPendiente()throws Exception {
        assertThat(puntos().size()).isZero();assertThat(contar("punto_entrega")).isZero();assertThat(problemas()).contains("PUNTOS_PENDIENTES","DISPONIBILIDAD_PUNTO_PENDIENTE");
        aceptar(post(admin,ruta(),alta(" p_uno ",List.of())));String punto=primerId();assertThat(puntos().get(0).get("codigo").asString()).isEqualTo("P_UNO");assertThat(puntos().get(0).get("disponibilidadOperativa").asString()).isEqualTo("PENDIENTE");assertThat(puntos().get(0).get("sucursales").size()).isZero();
        assertThat(problemas()).contains("PUNTOS_PENDIENTES","DISPONIBILIDAD_PUNTO_PENDIENTE");
