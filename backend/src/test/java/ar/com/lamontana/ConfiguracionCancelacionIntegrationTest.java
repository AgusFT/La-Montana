...
class ConfiguracionCancelacionIntegrationTest {
    ...
    @Test void correoRealCancelacionConcurrenteSnapshotHistorialReinicioYReplaySinAfectarNuevoBorrador() throws Exception {
        ...
        var solicitud=solicitud(2,CLAVE);var emitida=post(admin,ruta(id,"solicitar"),solicitud);status(emitida,200);
        ...
        for(String campo:List.of("codigoPublico","numero","modelo","criterio","creadaEn","actor"))assertThat(cancelada.get(campo)).isEqualTo(borrador.get(campo));
        assertThat(cancelada.get("version").asLong()).isEqualTo(3);assertThat(cancelada.get("canceladaEn").isNull()).isFalse();
        ...
        assertThat(eventos("BORRADOR_CANCELADO")).isEqualTo(1);assertThat(contar("comprobante_configuracion")).isEqualTo(3);
