...
class ConfiguracionCancelacionIntegrationTest {
    ...
    @Test void correoRealCancelacionConcurrenteSnapshotHistorialReinicioYReplaySinAfectarNuevoBorrador() throws Exception {
        ...
        var solicitud=solicitud(4,CLAVE);var emitida=post(admin,ruta(id,"solicitar"),solicitud);status(emitida,200);
        ...
        assertThat(cancelada.get("version").asLong()).isEqualTo(5);assertThat(cancelada.get("canceladaEn").isNull()).isFalse();
        ...
        assertThat(eventos("BORRADOR_CANCELADO")).isEqualTo(1);assertThat(contar("comprobante_configuracion")).isEqualTo(5);
        ...
        var modificarCancelada=enviar(admin,"PUT","/api/admin/configuracion/borradores/"+id+"/recursos",Map.of("operacion",UUID.randomUUID().toString(),"version",5,"metodoAsignacion","MANUAL","serviciosPorSucursal",List.of()));status(modificarCancelada,409);
        status(enviar(admin,"PUT","/api/admin/configuracion/borradores/"+id+"/entrega",Map.of("operacion",UUID.randomUUID().toString(),"version",5,"preparacionHoras","3","modalidades",List.of("RETIRO_SUCURSAL"),"horariosPorSucursal",List.of())),409);
