...
class ConfiguracionCancelacionIntegrationTest {
    ...
    @Test void correoRealCancelacionConcurrenteSnapshotHistorialReinicioYReplaySinAfectarNuevoBorrador() throws Exception {
        ...
        var entrega=enviar(admin,"PUT","/api/admin/configuracion/borradores/"+id+"/entrega",Map.of("operacion",UUID.randomUUID().toString(),"version",3,"preparacionHoras","2","trasladoHoras","1","modalidades",List.of("RETIRO_SUCURSAL"),"horariosPorSucursal",List.of(Map.of("sucursal",JSON.readTree(sucursal.body()).get("codigoPublico").asString(),"dias",List.of(Map.of("dia",1,"habilitado",true,"apertura","09:00","cierre","18:00"))))));
        status(entrega,200);borrador=JSON.readTree(entrega.body());
        var definicionPunto=new HashMap<String,Object>(Map.of("operacion",UUID.randomUUID().toString(),"version",4,"codigo","PUNTO","nombre","Punto conservado","calle","Calle","numero","456","localidad","Ciudad","provincia","Provincia","codigoPostal","1234","zonaHoraria","America/Argentina/Buenos_Aires"));
        definicionPunto.put("sucursales",List.of(Map.of("sucursal",JSON.readTree(sucursal.body()).get("codigoPublico").asString(),"costo","0","habilitado",true,"franjas",List.of(Map.of("dia",1,"apertura","17:00","cierre","18:00","capacidadPedidos",3,"habilitada",true)))));
        var punto=post(admin,"/api/admin/configuracion/borradores/"+id+"/entrega/puntos",definicionPunto);status(punto,200);borrador=JSON.readTree(punto.body());
        var solicitud=solicitud(5,CLAVE);var emitida=post(admin,ruta(id,"solicitar"),solicitud);status(emitida,200);
        ...
        for(String campo:List.of("codigoPublico","numero","modelo","criterio","creadaEn","actor","recursos","entrega"))assertThat(cancelada.get(campo)).isEqualTo(borrador.get(campo));
        assertThat(cancelada.get("version").asLong()).isEqualTo(6);assertThat(cancelada.get("canceladaEn").isNull()).isFalse();
        ...
        assertThat(eventos("BORRADOR_CANCELADO")).isEqualTo(1);assertThat(contar("comprobante_configuracion")).isEqualTo(6);
        ...
        assertThat(nuevo.get("entrega").get("preparacionHoras").isNull()).isTrue();assertThat(nuevo.get("entrega").get("trasladoHoras").isNull()).isTrue();assertThat(nuevo.get("entrega").get("modalidades").size()).isZero();assertThat(nuevo.get("entrega").get("horariosPorSucursal").size()).isZero();
        assertThat(nuevo.get("entrega").get("puntos").size()).isZero();
        var modificarCancelada=enviar(admin,"PUT","/api/admin/configuracion/borradores/"+id+"/recursos",Map.of("operacion",UUID.randomUUID().toString(),"version",6,"metodoAsignacion","MANUAL","serviciosPorSucursal",List.of()));status(modificarCancelada,409);
        status(enviar(admin,"PUT","/api/admin/configuracion/borradores/"+id+"/entrega",Map.of("operacion",UUID.randomUUID().toString(),"version",6,"preparacionHoras","3","modalidades",List.of("RETIRO_SUCURSAL"),"horariosPorSucursal",List.of())),409);
        var editarPunto=new HashMap<>(definicionPunto);editarPunto.remove("codigo");editarPunto.put("operacion",UUID.randomUUID().toString());editarPunto.put("version",6);
        status(enviar(admin,"PUT","/api/admin/configuracion/borradores/"+id+"/entrega/puntos/"+cancelada.get("entrega").get("puntos").get(0).get("codigoPublico").asString(),editarPunto),409);
