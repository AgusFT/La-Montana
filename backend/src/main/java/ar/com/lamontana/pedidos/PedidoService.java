...
public class PedidoService {
 ...
 @Transactional(readOnly=true,isolation=Isolation.REPEATABLE_READ) public Detalle detalle(UUID codigo,String correo,boolean interno){
  var a=actor(correo,interno);var rows=jdbc.query("SELECT p.*,c.codigo_publico AS cotizacion,c.oferta,s.codigo_publico AS sucursal,v.codigo_publico AS config,v.numero_version FROM lamontana.pedido p JOIN lamontana.cotizacion c USING(id_cotizacion) JOIN lamontana.sucursal s ON s.id_sucursal=p.id_sucursal JOIN lamontana.configuracion_version v ON v.id_configuracion_version=p.id_configuracion_version WHERE p.codigo_publico=? AND (? OR p.id_usuario_creador=?)",(r,n)->{
   ...
   return new Detalle(codigo,id,r.getObject("cotizacion",UUID.class),r.getString("estado"),r.getTimestamp("confirmada_en").toInstant(),r.getTimestamp("aprobada_en")==null?null:r.getTimestamp("aprobada_en").toInstant(),r.getObject("config",UUID.class),r.getLong("numero_version"),json.readValue(r.getString("oferta"),CotizacionService.Oferta.class),json.readValue(r.getString("condiciones_finales"),Condiciones.class),reserva,r.getString("receptor")==null?null:new PedidoController.Contacto(r.getString("receptor"),r.getString("telefono")),files,history,r.getLong("version"),jdbc.queryForObject("SELECT estado FROM lamontana.reserva_entrega WHERE id_pedido=?",String.class,id),r.getTimestamp("finalizada_en")==null?null:r.getTimestamp("finalizada_en").toInstant());
 ...
 @Transactional(readOnly=true,isolation=Isolation.REPEATABLE_READ) public Pagina listar(UUID branch,int pagina,String estado,String correo){
  ...
  if(estado!=null&&!Set.of("PENDIENTE_REVISION","APROBADO","RECHAZADO","CANCELADO").contains(estado))throw error(HttpStatus.BAD_REQUEST,"Estado de pedido inválido.");
