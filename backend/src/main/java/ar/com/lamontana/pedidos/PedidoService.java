...
public class PedidoService {
 ...
 @Transactional public Revision revisar(UUID quote,String correo){bloquear();actor(correo,false);var q=cotizaciones.detalle(quote,correo);return evaluar(q);}
 private Revision evaluar(CotizacionService.Detalle q){
  if(q.estado().equals("CONFIRMADA"))return new Revision(jdbc.queryForObject("SELECT p.codigo_publico FROM lamontana.pedido p JOIN lamontana.cotizacion c USING(id_cotizacion) WHERE c.codigo_publico=?",UUID.class,q.codigoPublico()),q.version(),null,0,null,false,List.of(),null,null,List.of());
  ...
  var franja=disponible(o,b.entrega(),estimacion,ahora,c.sucursales().stream().filter(s->s.codigoPublico().equals(branch)).findFirst().orElseThrow());
 ...
 private Franja disponible(CotizacionService.Oferta o,EntregaRepositorio.Entrega e,EvaluadorCalendario.Simulacion estimacion,Instant ahora,OfertaOperativaService.Sucursal sucursalActual){
  ...
  var v=franjas.siguiente(estimacion.disponibleDesde(),zona,slots,EvaluadorCalendario.limite(ahora,zona),w->ocupadas(o.modalidad(),dest,branch,w)<w.cupoConfigurado());
  return new Franja(o.modalidad(),dest,branch,nombre,zona,v.fecha(),v.apertura(),v.cierre(),v.desde(),v.hasta(),v.cupoConfigurado(),v.cupoConfigurado()-ocupadas(o.modalidad(),dest,branch,v),o.costoEntrega());
 }
 private int ocupadas(Modalidad modo,UUID dest,UUID branch,EvaluadorFranjas.Ventana v){return jdbc.queryForObject("SELECT lamontana.reservas_ocupadas(?,?,?,?::date,?::time,?::time,?,?)",Integer.class,modo.name(),dest,branch,v.fecha().toString(),v.apertura(),v.cierre(),Timestamp.from(v.desde()),Timestamp.from(v.hasta()));}
 @Transactional public Detalle confirmar(UUID quote,PedidoController.Confirmar in,String correo){
  bloquear();var a=actor(correo,false);var q=cotizaciones.detalle(quote,correo);String command=huella(List.of(quote,in));
 ...
 @Transactional(readOnly=true,isolation=Isolation.REPEATABLE_READ) public Detalle detalle(UUID codigo,String correo,boolean interno){
  var a=actor(correo,interno);var rows=jdbc.query("SELECT p.*,c.codigo_publico AS cotizacion,c.oferta,s.codigo_publico AS sucursal,v.codigo_publico AS config,v.numero_version FROM lamontana.pedido p JOIN lamontana.cotizacion c USING(id_cotizacion) JOIN lamontana.sucursal s ON s.id_sucursal=p.id_sucursal JOIN lamontana.configuracion_version v ON v.id_configuracion_version=p.id_configuracion_version WHERE p.codigo_publico=? AND (? OR p.id_usuario_creador=?)",(r,n)->{
   if(interno)organizacion.sucursalAutorizada(correo,r.getObject("sucursal",UUID.class));long id=r.getLong("id_pedido");
   var reserva=jdbc.queryForObject("SELECT * FROM lamontana.reserva_entrega WHERE id_pedido=?",(rs,nr)->new Franja(Modalidad.valueOf(rs.getString("modalidad")),rs.getObject("destino_publico",UUID.class),rs.getObject("origen_publico",UUID.class),rs.getString("nombre_destino"),rs.getString("zona_horaria"),rs.getDate("fecha_local").toLocalDate(),rs.getTime("apertura").toLocalTime().toString(),rs.getTime("cierre").toLocalTime().toString(),rs.getTimestamp("franja_desde").toInstant(),rs.getTimestamp("franja_hasta").toInstant(),rs.getInt("cupo_confirmado"),0,rs.getBigDecimal("costo").toPlainString()),id);
   var files=jdbc.query("SELECT ci.codigo_publico,a.codigo_publico,a.nombre_original,a.sha256,a.cantidad_bytes,a.cantidad_paginas,v.fecha FROM lamontana.pedido_item i JOIN lamontana.cotizacion_item ci USING(id_cotizacion_item) JOIN lamontana.archivo_almacenado a USING(id_archivo_almacenado) JOIN lamontana.aceptacion_vista_previa v USING(id_aceptacion_vista_previa) WHERE i.id_pedido=? ORDER BY ci.orden",(rs,nr)->new Pdf(rs.getObject(1,UUID.class),rs.getObject(2,UUID.class),rs.getString(3),rs.getString(4),rs.getLong(5),rs.getInt(6),rs.getTimestamp(7).toInstant()),id);
   var history=jdbc.query("SELECT estado_destino,motivo,fecha FROM lamontana.historial_estado_pedido WHERE id_pedido=? ORDER BY id_evento",(rs,nr)->new Evento(rs.getString(1),rs.getString(2),rs.getTimestamp(3).toInstant()),id);
   return new Detalle(codigo,id,r.getObject("cotizacion",UUID.class),r.getString("estado"),r.getTimestamp("confirmada_en").toInstant(),r.getTimestamp("aprobada_en")==null?null:r.getTimestamp("aprobada_en").toInstant(),r.getObject("config",UUID.class),r.getLong("numero_version"),json.readValue(r.getString("oferta"),CotizacionService.Oferta.class),json.readValue(r.getString("condiciones_finales"),Condiciones.class),reserva,r.getString("receptor")==null?null:new PedidoController.Contacto(r.getString("receptor"),r.getString("telefono")),files,history);
 ...
 @Transactional(readOnly=true,isolation=Isolation.REPEATABLE_READ) public Pagina listar(UUID branch,int pagina,String correo){
  var a=actor(correo,branch!=null);if(branch!=null)organizacion.sucursalAutorizada(correo,branch);if(pagina<0||pagina>100000)throw error(HttpStatus.BAD_REQUEST,"Página inválida.");
  String from=" FROM lamontana.pedido p JOIN lamontana.sucursal s USING(id_sucursal) JOIN lamontana.cotizacion c USING(id_cotizacion) WHERE "+(branch==null?"p.id_usuario_creador=?":"s.codigo_publico=?");Object filtro=branch==null?a.id():branch;
  long count=jdbc.queryForObject("SELECT count(*)"+from,Long.class,filtro);
  var ids=jdbc.query("SELECT p.codigo_publico,p.id_pedido,p.estado,p.confirmada_en,s.nombre,p.total,c.oferta->>'modalidad' AS modalidad"+from+" ORDER BY p.id_pedido DESC LIMIT 25 OFFSET ?",(r,n)->new Resumen(r.getObject(1,UUID.class),r.getLong(2),r.getString(3),r.getTimestamp(4).toInstant(),r.getString(5),r.getBigDecimal(6).toPlainString(),Modalidad.valueOf(r.getString(7))),filtro,(long)pagina*25);
