...
public class ProgramacionConfiguracionService {
    ...
    public ConfiguracionService.Programacion confirmar(UUID destino,Confirmar i,String actor){
        ...
        jdbc.update("INSERT INTO lamontana.programacion_configuracion(id_configuracion_version,id_actor,id_operacion,fecha_confirmacion,fecha_programada,zona_horaria,motivo) VALUES (?,?,?,?,?,?,?)",d.idConfiguracion(),d.actor(),i.operacion(),ahora,Timestamp.from(prevista),i.decision().zonaHoraria(),i.decision().revision().motivo().strip());
