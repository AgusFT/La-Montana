...
public class EntregaRepositorio {
    ...
    public EntregaRepositorio(JdbcTemplate jdbc,PuntosRepositorio puntos){this.jdbc=jdbc;this.puntos=puntos;}
    public record Horario(UUID sucursal,String zonaHoraria,List<Dia> dias) {}
    public record Entrega(String preparacionHoras,String trasladoHoras,List<Modalidad> modalidades,List<Horario> horariosPorSucursal,List<PuntosRepositorio.Punto> puntos) {}
    public Entrega leer(long config) {
        ...
        return new Entrega(tiempos.isEmpty()?null:tiempos.get(0).preparacion(),tiempos.isEmpty()?null:tiempos.get(0).traslado(),modalidades,horarios,puntos.leer(config));
    }
