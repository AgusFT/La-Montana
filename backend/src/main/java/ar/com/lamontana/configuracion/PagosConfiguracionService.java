...
public class PagosConfiguracionService {
    ...
    public ConfiguracionService.Borrador guardar(UUID codigo,GuardarPagos input,String correo) {
        ...
        var valores=validar(borrador,input.pagos());Pagos p=input.pagos();
    ...
    private Valores validar(ConfiguracionService.Borrador b,Pagos p) {
        ...
        if(b.modelo()==Modelo.MANUAL) {
            ...
            exigir(transferencia,"El modelo condicional requiere transferencia para acreditar el requisito económico.");
            switch(b.criterio()) {
                case PAGO_PREVIO -> {
                ...
                case MONTO_TOTAL -> exigir(p.exigirSena()&&p.condicionSena()==CondicionSena.SUPERAR_UMBRAL_APROBACION,"La condición por monto exige la seña sobre su umbral, después de la aprobación humana.");
            }
        }
        BigDecimal umbralAprobacion=b.criterio()==Criterio.MONTO_TOTAL?decimalPositivo(p.umbralAprobacion(),2,"El umbral de aprobación"):null;
        if(!p.exigirSena()) {
